...
const STATUS_STYLES = {
  emerald: { badge: "bg-brand-primary/20 border-brand-primary/40 text-brand-primary-tint", ring: "border-brand-primary/30", dot: "bg-brand-primary-tint" },
  amber:   { badge: "bg-brand-secondary/20 border-brand-secondary/40 text-brand-secondary-tint",       ring: "border-brand-secondary/40",  dot: "bg-brand-secondary-tint animate-pulse" },
  rose:    { badge: "bg-red-900/40 border-red-700/50 text-red-400",           ring: "border-red-700/40",   dot: "bg-red-400 animate-pulse" },
  red:     { badge: "bg-red-950/50 border-red-800/50 text-red-400",              ring: "border-red-800/40",    dot: "bg-red-500" },
  slate:   { badge: "bg-brand-200/10 border-brand-200/15 text-brand-200/50",        ring: "border-brand-200/15",  dot: "bg-brand-200/40" },
};
...
function AlertaModal({ poliza, cliente, onClose, onConfirm, loading }) {
  ...
  const cfgs = {
    vencida: {
      headerBg:   "bg-red-950/70 border-b border-red-800/50",
      modalBorder:"border-red-800/50",
      ...
      body: (
        <div className="space-y-4 text-sm text-brand-200/75">
          <p>La <span className="font-bold text-white">última cuota</span> venció y <span className="font-bold text-red-300">no fue pagada</span>.</p>
          <div className="rounded-xl border border-red-900/50 bg-red-950/20 p-4">
            <p className="text-xs font-bold uppercase tracking-wider text-red-400 mb-3">Antes de cobrar verificá:</p>
            <ul className="space-y-2 text-sm text-red-300/80 list-disc pl-4">
              <li>Que la póliza <strong className="text-red-300">no esté dada de baja</strong> en la compañía</li>
              <li>Que el cliente <strong className="text-red-300">no tuvo siniestros</strong> durante el período vencido</li>
            </ul>
          </div>
          <p className="text-xs text-brand-200/50">Si todo está en orden, podés registrar el cobro a continuación.</p>
        </div>
      ),
    ...
    baja_reciente: {
      headerBg:   "bg-brand-secondary/15 border-b border-brand-secondary/40",
      modalBorder:"border-brand-secondary/40",
      iconBg:     "bg-brand-secondary/20",
      iconColor:  "text-brand-secondary-tint",
      Icon:       HiExclamation,
      title:      "Póliza dada de baja recientemente",
      titleColor: "text-brand-secondary-tint",
      body: (
        <div className="space-y-4 text-sm text-brand-200/75">
          <p>Esta póliza fue <span className="font-bold text-brand-secondary-tint">cancelada</span> y <strong className="text-white">no se puede cobrar</strong>.</p>
          <div className="rounded-xl border border-brand-secondary/40 bg-brand-secondary/10 p-4">
            <div className="grid grid-cols-2 gap-3 mb-1">
              <div>
                <p className="text-[11px] uppercase tracking-wider text-brand-secondary-tint mb-1">Fecha de baja</p>
                <p className="text-base font-semibold text-brand-secondary-tint">{st.fechaBaja?.format("DD/MM/YYYY") || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-wider text-brand-secondary-tint mb-1">Días transcurridos</p>
                <p className="text-base font-semibold text-brand-secondary-tint">{st.diasBaja != null ? fmtDias(st.diasBaja) : "—"}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-wider text-brand-secondary-tint mb-1">Patente</p>
                <p className="text-base font-semibold text-brand-secondary-tint font-mono">{poliza?.patente || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-wider text-brand-secondary-tint mb-1">Compañía</p>
                <p className="text-base font-semibold text-brand-secondary-tint">{poliza?.compania || "—"}</p>
              </div>
            </div>
          </div>
          <div className="rounded-xl border border-brand-200/15 bg-brand-200/[0.04] p-4">
            <p className="text-xs font-bold uppercase tracking-wider text-brand-200/50 mb-3">Para regularizar:</p>
            <ul className="space-y-2 text-sm text-brand-200/50 list-disc pl-4">
              <li>Contactar a la compañía para emitir una <strong className="text-brand-200/90">nueva póliza</strong></li>
      ...
      btnClass:  "bg-brand-primary hover:bg-brand-primary-deep text-white",
      action:    "nueva",
    },
    cancelada: {
      headerBg:   "bg-brand-200/8 border-b border-brand-200/15",
      modalBorder:"border-brand-200/15",
      iconBg:     "bg-brand-200/10",
      iconColor:  "text-brand-200/50",
      Icon:       HiBan,
      title:      "Póliza cancelada",
      titleColor: "text-brand-200/75",
      body: (
        <div className="space-y-3 text-sm text-brand-200/50">
          <p>Esta póliza fue <span className="font-bold text-brand-200/75">cancelada</span> y no tiene acciones disponibles.</p>
          <p className="text-xs text-brand-200/40">Si el cliente quiere volver a asegurarse, hay que emitir una póliza completamente nueva desde el flujo de solicitudes.</p>
        </div>
      ),
      btnLabel:  null,
      action:    null,
  ...
  return createPortal(
    ...
    >
      ...
      <motion.div
        ...
        className={`rounded-2xl border shadow-2xl overflow-hidden bg-brand-card-dark ${cfg.modalBorder}`}
      >
        {/* Header */}
        <div className={`px-5 py-4 flex items-center gap-3 ${cfg.headerBg}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-brand-200/50 mt-0.5 font-mono truncate">
              {poliza?.patente || "—"} · {poliza?.compania || "—"}
            </p>
          </div>
          <button onClick={onClose}
            className="h-9 w-9 rounded-xl flex items-center justify-center text-brand-200/40 hover:text-brand-200/75 hover:bg-brand-200/8 transition-colors shrink-0">
        ...
        <div className="px-5 pb-5 flex flex-col gap-2.5">
          ...
          <button onClick={onClose}
            className="w-full py-3 rounded-xl text-sm font-medium text-brand-200/50 hover:text-brand-200/90 hover:bg-brand-200/8 transition-colors border border-brand-200/10">
            Cerrar
          </button>
        </div>
      </motion.div>
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <form onSubmit={handleSubmit}
        className="flex items-center gap-3 bg-brand-200/[0.04] border border-brand-200/10 focus-within:border-brand-primary/50 rounded-2xl px-5 py-4 transition-colors duration-200">
        {busy
          ? <span className="w-6 h-6 rounded-full border-2 border-brand-primary/50 border-t-transparent animate-spin shrink-0" />
          : <HiSearch className="w-6 h-6 text-brand-200/40 shrink-0" />}
        <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Escape") limpiar(); }}
          placeholder="Buscar por DNI o patente…"
          className="flex-1 bg-transparent text-lg text-brand-200 placeholder:text-brand-200/40 outline-none min-w-0" />
        {query && (
          <button type="button" onClick={limpiar}
            className="p-1.5 rounded-lg hover:bg-brand-200/10 text-brand-200/40 hover:text-brand-200/75 transition-colors shrink-0">
            <HiX className="w-5 h-5" />
          </button>
        )}
        <button type="submit" disabled={busy || !query}
          className="shrink-0 px-5 py-2.5 rounded-xl bg-brand-primary hover:bg-brand-primary-deep disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold transition-colors">
          Buscar
        </button>
      </form>
      ...
      <AnimatePresence>
        {recientes.length > 0 && !cliente && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <span className="text-xs text-brand-200/30">Recientes:</span>
            {recientes.map((q) => (
              <button key={q} type="button" onClick={() => { setQuery(String(q)); handleSubmit(null, q); }}
                className="px-3 py-1.5 rounded-xl bg-brand-200/8 border border-brand-200/15 hover:bg-brand-200/20 text-brand-200/50 text-xs font-mono transition-colors">
                {q}
              </button>
            ))}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
            ...
            <div className="flex items-center justify-between gap-3 px-1">
              <div className="flex items-center gap-3">
                <div className="h-11 w-11 rounded-full bg-brand-primary/20 border border-brand-primary/30 flex items-center justify-center text-brand-primary-tint font-bold text-lg shrink-0">
                  {String(cliente?.nombre_apellido || "?")[0]?.toUpperCase()}
                </div>
                <div>
                  <div className="text-base font-semibold text-brand-200 flex items-center gap-2 flex-wrap">
                    {cliente?.nombre_apellido || "Cliente"}
                  </div>
                  <div className="text-xs text-brand-200/40 font-mono mt-0.5">DNI {cliente?.dni}</div>
                </div>
              </div>
              <button type="button" onClick={limpiar}
                className="px-3 py-1.5 rounded-xl bg-brand-200/8 border border-brand-200/15 hover:bg-brand-200/15 text-brand-200/50 text-xs transition-colors">
                Nueva búsqueda
              </button>
            </div>
              ...
              {polizas.map((p, idx) => {
                ...
                return (
                  ...
                  <motion.button type="button"
                    ...
                    className={`group/card w-full text-left rounded-2xl border p-4 transition-colors duration-150 bg-brand-200/[0.03] hover:bg-brand-200/[0.05] cursor-pointer ${styles.ring}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-mono text-sm font-bold text-white tracking-widest bg-brand-200/10 px-2.5 py-1 rounded-lg">
                            {p?.patente || "—"}
                          </span>
                          <span className="text-sm text-brand-200/75 truncate">{p?.compania || "—"}</span>
                        </div>
                        {p?.modelo && <div className="text-xs text-brand-200/40 mt-2 truncate">{p.modelo}</div>}
                        <div className="mt-3">
                      ...
                      <div className="flex flex-col items-end justify-between gap-2 shrink-0 self-stretch">
                        {needsAlert
                          ? <span className="inline-flex items-center gap-1 text-[10px] text-brand-200/40"><HiExclamationCircle className="w-3.5 h-3.5" /> Ver detalle</span>
                          : <span className="text-[10px] text-brand-200/30">Ver cuotas</span>}
                        <HiOutlineChevronRight className="w-5 h-5 text-brand-200/30 group-hover/card:text-brand-200/50 transition-colors" />
                      </div>
                    </div>
                  </motion.button>
                  {puedeRenovar && (
                    <button type="button" onClick={() => abrirRenovar(p)}
                      className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-brand-primary hover:bg-brand-primary-deep text-white text-sm font-bold py-2.5 transition-colors">
            ...
            {polizas.length === 0 && (
              <p className="text-sm text-brand-200/40 px-1">No hay pólizas para este cliente.</p>
            )}
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {renovarTarget && (
          <motion.div
            ...
            <motion.div
              ...
              className="relative z-10 w-full max-w-md rounded-2xl border border-brand-primary/40 bg-brand-card-dark shadow-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-brand-200/10 flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-brand-primary/15 flex items-center justify-center shrink-0">
                  <HiRefresh className="w-5 h-5 text-brand-primary-tint" />
                </div>
                <div className="min-w-0">
                  <div className="text-lg font-bold text-white">Renovar póliza</div>
                  <div className="text-xs text-brand-200/50 font-mono truncate">
              ...
              <div className="px-5 py-4 space-y-2.5 text-sm">
                <div className="flex justify-between gap-3">
                  <span className="text-brand-200/50">Compañía</span>
                  <span className="font-semibold text-white text-right">{renovarTarget?.compania || "—"} <span className="text-brand-200/40 font-normal">(la misma)</span></span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-brand-200/50">Pago</span>
                  <span className="font-semibold text-white">hoy {hoyTxt}</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-brand-200/50">Cobertura desde</span>
                  <span className="font-semibold text-brand-primary-tint">{coberturaTxt}</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-brand-200/50">Vencimiento</span>
                  <span className="font-semibold text-white text-right">se mantiene el día de siempre</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-brand-200/50">Fotos y documentos</span>
                  <span className="font-semibold text-brand-primary-tint text-right">se mueven a la nueva</span>
                </div>
                <div className="flex justify-between gap-3">
                  <span className="text-brand-200/50">Póliza actual</span>
                  <span className="font-semibold text-white text-right">pasa a finalizada</span>
                </div>
                <p className="text-xs text-brand-200/40 pt-1">
              ...
              <div className="px-5 pb-5 flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
                <button type="button" onClick={() => setRenovarTarget(null)} disabled={renovando}
                  className="rounded-xl border border-brand-200/15 bg-brand-200/10 hover:bg-brand-200/15 px-4 py-2 text-sm text-brand-200/90 transition-colors disabled:opacity-50">
                  Cancelar
                </button>
                <button type="button" onClick={confirmarRenovar} disabled={renovando}
                  className="rounded-xl bg-brand-primary hover:bg-brand-primary-deep px-5 py-2 text-sm font-bold text-white transition-colors disabled:opacity-50 inline-flex items-center justify-center gap-2">
                  {renovando
