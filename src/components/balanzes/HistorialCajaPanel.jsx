/* src/components/balanzes/HistorialCajaPanel.jsx
   Historial de INGRESOS o EGRESOS de caja por oficina (unifica los 2 paneles).
   Prop `tipo`: "ingreso" | "egreso".

   ✅ DESCARGA: botón "Balance completo" → backend reporte-completo (Resumen + Ingresos + Egresos, xlsx).
   La lista se trae por ingresos/historial/ o egresos/historial/ (JSON paginado).
   Admin ve todas las oficinas. Empleado solo la suya.
*/
...
// Config por tipo (endpoint + colores + textos). Clases Tailwind COMPLETAS
// (no interpoladas) para que no las purgue el build.
const CONF = {
  ingreso: {
    endpoint: "ingresos/historial/",
    totalIcon: HiCash,
    totalColor: "text-emerald-400",
    efectivoColor: "text-emerald-300",
    btn: "bg-emerald-700 hover:bg-emerald-600",
    spinner: "border-emerald-400/40 border-t-emerald-400",
    efeBadge: "bg-emerald-900/40 border-emerald-700/50 text-emerald-400",
    placeholder: "Buscar por descripción, pagador, categoría, cuenta…",
    label: "ingresos",
    tag: "HistorialIngresosPanel",
  },
  egreso: {
    endpoint: "egresos/historial/",
    totalIcon: HiTrendingDown,
    totalColor: "text-rose-400",
    efectivoColor: "text-rose-300",
    btn: "bg-rose-700 hover:bg-rose-600",
    spinner: "border-rose-400/40 border-t-rose-400",
    efeBadge: "bg-rose-900/40 border-rose-700/50 text-rose-400",
    placeholder: "Buscar por descripción o categoría…",
    label: "egresos",
    tag: "HistorialEgresosPanel",
  },
};

function FormaPagoBadge({ forma, efeBadge }) {
  const f = (forma || "EFECTIVO").toUpperCase();
  if (f === "EFECTIVO")
    return <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full border ${efeBadge}`}>💵 Efectivo</span>;
  if (f === "TRANSFERENCIA")
...
function buildFilenameSuffix({ desde, hasta, oficinaLabel }) {
  const safe = (s) =>
    String(s ?? "")
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
...
export default function HistorialCajaPanel({ tipo = "ingreso", oficinasAdmin = [], oficinaProp }) {
  ...
  const cargar = useCallback(async (p = 1) => {
    ...
    try {
      ...
      console.error(`[${conf.tag}] Error al cargar:`, err);
      const status = err?.response?.status;
      if (status === 401 || status === 403) {
        toast.error("Tu sesión expiró. Volvé a iniciar sesión.");
      } else {
        toast.error(`No se pudieron cargar los ${conf.label}.`);
      }
      setItems([]); setCount(0); setNext(null); setPrev(null);
    } finally {
      setLoading(false);
    }
  }, [conf.endpoint, conf.tag, conf.label, oficina, desde, hasta, formaPago, q]);

  useEffect(() => { setPage(1); cargar(1); }, [oficina, desde, hasta, formaPago, q]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ── Descarga: SIEMPRE el reporte-completo (Resumen + Ingresos + Egresos, xlsx) ──
  const handleExport = async () => {
    if (exporting) return;
    setExporting(true);
    const toastId = toast.loading("Generando Excel…");
    const oficinaLabel = oficina === "ALL" ? "" : (oficinasMap[String(oficina)] || String(oficina));
    try {
      const res = await api.get("reporte-completo/", {
        params: { oficina: oficina !== "ALL" ? oficina : undefined, desde, hasta },
        responseType: "blob",
        timeout: 60_000,
      });
      const ct = String(res.headers?.["content-type"] || "").toLowerCase();
      const esArchivo = ct.includes("spreadsheet") || ct.includes("xlsx") || ct.includes("octet");
      if (!esArchivo) {
      ...
      const suffix = buildFilenameSuffix({ desde, hasta, oficinaLabel });
      const filename = `Balance_Completo_${suffix}.xlsx`;
      ...
      console.error(`[${conf.tag}] Error al exportar:`, err);
      toast.dismiss(toastId);
      const status = err?.response?.status;
      const detail = err?.response?.data?.detail || err?.response?.data?.error || err?.message || "Error desconocido";
      if (status === 401 || status === 403) toast.error("Tu sesión expiró. Volvé a iniciar sesión.");
      else if (status === 404) toast.error("Endpoint no encontrado. Avisale al admin.");
      else if (status === 500) toast.error("Error del servidor. Revisá los logs.");
      else toast.error(`No se pudo descargar: ${detail}`);
    } finally {
      setExporting(false);
    }
  };
  ...
  const kpis = useMemo(() => {
    ...
    const efectivo = items.filter(i => (i.forma_pago || "").toUpperCase() === "EFECTIVO").reduce((s, i) => s + Number(i.monto || 0), 0);
    const transf   = items.filter(i => (i.forma_pago || "").toUpperCase() === "TRANSFERENCIA").reduce((s, i) => s + Number(i.monto || 0), 0);
    return { total, efectivo, transf, cantidad: items.length };
  }, [items]);

  const TotalIcon = conf.totalIcon;

  // Grilla: ingresos tiene 2 columnas extra (pagador/cuenta + cta destino).
  const gridCols = esIngreso
    ? (isWebAdmin
        ? "grid-cols-[0.8fr_1.6fr_1fr_0.8fr_1.2fr_1fr_0.8fr_0.8fr]"
        : "grid-cols-[0.8fr_1.6fr_1fr_0.8fr_1.2fr_1fr_0.8fr]")
    : (isWebAdmin
        ? "grid-cols-[0.9fr_2.2fr_1.2fr_0.9fr_0.9fr_0.9fr]"
        : "grid-cols-[0.9fr_2.2fr_1.2fr_0.9fr_0.9fr]");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-4 space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
          ...
          <button onClick={handleExport} disabled={exporting || loading}
            title="Descargar balance completo (Resumen + Ingresos + Egresos) del período"
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl ${conf.btn} disabled:opacity-50 text-white text-sm font-semibold transition-colors`}>
            <HiDownload className={`w-4 h-4 ${exporting ? "animate-bounce" : ""}`} />
            {exporting ? "Descargando…" : "Balance completo"}
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto text-xs text-slate-500">
            {count > 0 ? `${count.toLocaleString("es-AR")} ${conf.label} encontrados` : ""}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total período", value: fmtMoney(kpis.total),    color: conf.totalColor,    icon: TotalIcon },
          { label: "Efectivo",      value: fmtMoney(kpis.efectivo), color: conf.efectivoColor, icon: HiCash },
      ...
      <div className="bg-slate-900/50 border border-slate-800/60 rounded-2xl overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-16">
            <span className={`w-8 h-8 rounded-full border-2 ${conf.spinner} animate-spin`} />
          </div>
        ) : items.length === 0 ? (
          <div className="py-16 text-center text-slate-500 text-sm">
            Sin {conf.label} para los filtros aplicados.
          </div>
        ) : (
          <>
            {/* Header */}
            <div className={`grid gap-2 px-4 py-3 bg-slate-950/60 border-b border-slate-800/60 text-[10px] uppercase tracking-wider text-slate-500 font-semibold ${gridCols}`}>
              <div>Fecha</div>
              <div>Descripción</div>
              <div>Categoría</div>
              <div>Forma</div>
              {esIngreso && <div>Pagado por / Cuenta</div>}
              {esIngreso && <div>Cta. destino</div>}
              ...
              <AnimatePresence>
                {items.map((item, idx) => (
                  <motion.div key={item.id || idx}
                    ...
                    className={`grid gap-2 px-4 py-3 hover:bg-slate-800/30 transition-colors text-sm ${gridCols}`}>
                    ...
                    <div className="flex items-center">
                      <FormaPagoBadge forma={item.forma_pago} efeBadge={conf.efeBadge} />
                    </div>
                    ...
                    <div className="flex items-center justify-end">
                      <span className={`font-bold tabular-nums ${conf.totalColor}`}>{fmtMoney(item.monto)}</span>
                    </div>
                  </motion.div>
                ))}
              </AnimatePresence>
